import discord, json, math, os, games, asyncio, random, main_controller, threading, urllib, leagues, datetime
...
class StartGameCommand(Command):
    name = "startgame"
    template = "r.startgame [away] [home] [innings]"
...
class ImportCommand(Command):
    name = "import"
    template = "r.import [onomancer collection URL]"
...
class SearchTeamsCommand(Command):
    name = "searchteams"
    template = "r.searchteams [searchterm]"
...
class MovePlayerCommand(Command):
    name = "moveplayer"
    template = """r.moveplayer 
    [team name]
    [player name]
    [new lineup/rotation position number] (indexed with 1 being the top)"""
    description = "Moves a player within your lineup or rotation. If you want to instead move a player from your rotation to your lineup or vice versa, use r.swapsection instead. Requires team ownership and exact spelling of team name."
...
class AddPlayerCommand(Command):
    name = "addplayer"
    template = """r.addplayer pitcher (or r.addplayer batter)
    [team name]
    [player name]"""
...
class HelpCommand(Command):
    ...
    async def execute(self, msg, command):
        query = command.strip()
        if query == "":
            text = "Here's everything we know how to do; use `r.help [command]` for more info:"
            ...
            try:
                ...
                text = "Can't find that command, boss; try checking the list with `r.help`."
        await msg.channel.send(text)

class DeleteTeamCommand(Command):
    name = "deleteteam"
    template = "r.deleteteam [name]"
...
class AssignOwnerCommand(Command):
    name = "assignowner"
    template = "r.assignowner [mention] [team]"
...
class StartDraftCommand(Command):
 ...
 - use the command `d`, `draft`, or `r.draft` on your turn to draft someone
    """
...
class StartLeagueCommand(Command):
    name = "startleague"
    template = "r.startleague [league name]\n[games per hour]"
...
class LeagueDisplayCommand(Command):
    ...
    template = "r.leaguestandings [league name]"
...
class LeagueLeadersCommand(Command):
    name = "leagueleaders"
    template = "r.leagueleaders [league name]\n[stat name/abbreviation]"
...
class LeagueDivisionDisplayCommand(Command):
    ...
    template = "r.divisionstandings [league name]\n[team name]"
...
class LeagueWildcardCommand(Command):
    name = "leaguewildcard"
    template = "r.leaguewildcard [league name]"
...
class LeaguePauseCommand(Command):
    name = "pauseleague"
    template = "r.pauseleague [league name]"
...
class LeagueClaimCommand(Command):
    name = "claimleague"
    template = "r.claimleague [league name]"
...
class LeagueAddOwnersCommand(Command):
    name = "addleagueowner"
    template = "r.addleagueowner [league name]\n[user mentions]"
...
class LeagueScheduleCommand(Command):
    name = "leagueschedule"
    template = "r.leagueschedule [league name]"
...
class LeagueTeamScheduleCommand(Command):
    name = "teamschedule"
    template = "r.teamschedule [league name]\n[team name]"
...
class LeagueRegenerateScheduleCommand(Command):
    ...
    template = "r.leagueseasonreset [league name]"
...
class LeagueForceStopCommand(Command):
    ...
    template = "r.leagueforcestop [league name]"
...
async def on_message(msg):
    ...
    if msg.channel.id == config()["soulscream channel id"]:
        ...
        try:
            ...
            await msg.channel.send("Can't find that command, boss; try checking the list with `r.help`.")
